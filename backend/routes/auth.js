...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: user.id, email, name: user.name }, JWT_SECRET, { expiresIn: "7d" }); // Added name to payload
    res.json({ token });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
...
router.post("/profile-update", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) {
        // This is primarily for when the name is exactly the same
